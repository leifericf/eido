...
(defn layered
  ...
  [shape-node n opacity deform-fn seed]
  {:node/type :group
   :group/children
   (mapv (fn [i]
...
(defn watercolor
  ...
  Wraps (layered shape n opacity deform-fn seed).
  opts: :layers (30), :opacity (0.04), :amount (3.0), :seed (0)."
  [path-node opts]
  (let [layers  (get opts :layers 30)
        opacity (get opts :opacity 0.04)
        amount  (double (get opts :amount 3.0))
        seed    (get opts :seed 0)]
    (layered path-node layers opacity
      (fn [node _i s]

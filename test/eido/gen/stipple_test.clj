...
(deftest poisson-disk-test
  (testing "generates points within bounds"
    (let [pts (stipple/poisson-disk [0 0 100 100] {:min-dist 5 :seed 42})]
      (is (pos? (count pts)))
      (is (every? (fn [[x y]] (and (<= 0 x 100) (<= 0 y 100))) pts))))
  (testing "minimum distance between points"
    (let [min-dist 10
          pts (stipple/poisson-disk [0 0 100 100] {:min-dist min-dist :seed 42})]
      (is (every? (fn [[x1 y1]]
                    (every? (fn [[x2 y2]]
                              (or (and (= x1 x2) (= y1 y2))
  ...
  (testing "deterministic with seed"
    (is (= (stipple/poisson-disk [0 0 100 100] {:min-dist 5 :seed 42})
           (stipple/poisson-disk [0 0 100 100] {:min-dist 5 :seed 42})))))
